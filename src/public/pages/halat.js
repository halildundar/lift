...
export const HalatInit = async (route) => {
  onayKurumlar = await GetListOnayKurumlar();
  GetHalatlar();
  onSelectDosya();
  $("#save").on("click", async function (e) {
    $(".msg-area").html("");
    let filename = "WIRE ROPE";
    ...
    if (!Object.values(formData).some((item) => item == "")) {
      ...
      await AddSertifika("halat", formData);
      GetHalatlar();
  ...
  $("#update").on("click", async function (e) {
    ...
    if (!Object.values(formData).some((item) => item == "")) {
      ...
      await UpdateSertifika("halat", { id: selectedComp.id, ...formData });
      GetHalatlar();
      $(".spinner-area").addClass("!hidden");
    } else {
      $(".msg-area").html(
